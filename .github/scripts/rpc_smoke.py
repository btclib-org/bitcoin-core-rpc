...
# the private key 1: the one key in bitcoin published everywhere and owned
# by nobody. What it is for is an address no wallet of this node knows, so
# that the coinbase paid to it is a transaction only -txindex can answer
# for. The address is derived by the node from the descriptor below rather
# than encoded here, this script having no bitcoin library behind it and
# no business growing one: what it is testing is the rpc client
...
def node(
...
) -> Iterator[BitcoinCoreRpcClient]:  # pragma: no cover -- a regtest bitcoind
    ...
    client = BitcoinCoreRpcClient.from_chain(
        CHAIN, cookie_path=datadir / DATADIR_SUBDIR / ".cookie"
    )
...
) -> Iterator[BitcoinCoreRpcClient]:  # pragma: no cover -- any chain's bitcoind
...
) -> None:  # pragma: no cover -- the answer ending the wait needs a node
...
def stop(
    client: BitcoinCoreRpcClient, process: subprocess.Popen[bytes]
) -> None:  # pragma: no cover -- it stops a running bitcoind
...
) -> tuple[int, Mapping[str, Any]]:  # pragma: no cover -- a reply off the wire
...
) -> None:  # pragma: no cover -- only a node refuses a credential
...
def check_wallet_endpoint(
    client: BitcoinCoreRpcClient,
) -> None:  # pragma: no cover -- a node with two wallets loaded
...
def check_named_params(
    client: BitcoinCoreRpcClient,
) -> None:  # pragma: no cover -- the parameter structures Core reads
...
) -> None:  # pragma: no cover -- the amount comes from a node's wallet
...
def generate_chain(
    client: BitcoinCoreRpcClient,
) -> tuple[int, str, str]:  # pragma: no cover -- it generates blocks on a node
    ...
    # that could only be answered from the index. The node derives the
    # address: `getdescriptorinfo` appends the checksum `deriveaddresses`
    # requires, so the two calls are also a positional and a named
    # parameter reaching a method that computes something
    descriptor = client.call("getdescriptorinfo", [f"wpkh({FOREIGN_PUB_KEY})"])
    foreign = client.call("deriveaddresses", {"descriptor": descriptor["descriptor"]})[
        0
    ]
...
def check_serialization(
...
) -> Mapping[str, Any]:  # pragma: no cover -- the node's own serialization
    ...
    which is the widest reply this client asks for and therefore the one
    whose bounded read is worth exercising against a real body. Whether
    those octets are the transaction that was asked for is checked by
    asking the same method again at verbosity 1: the node hands back the id
    it computed and the same hex under a `hex` member, so a serialization
    that lost or gained an octet on the way here does not match.

    The node computes the id and not this script, which has no bitcoin
    library behind it and no business growing one -- what is under test is
    the rpc client, and a hash function written here would be a second
    thing that can be wrong.
    """
    raw = client.call("getrawtransaction", [tx_id])
    decoded: Mapping[str, Any] = client.call("getrawtransaction", [tx_id, 1])
    check(decoded["txid"] == tx_id, f"{claim}: {tx_id}")
    check(decoded["hex"] == raw, "the serialization is the one the node computed from")
    return decoded


def check_chain_answers(
    client: BitcoinCoreRpcClient, height: int, tx_id: str, foreign_coinbase: str
) -> None:  # pragma: no cover -- it asks about the chain generated here
    ...
    come back as a serialization checked against the node's own decoding of
    it, and the second of them is a coinbase no wallet knows, so `-txindex`
    is what answered it.
    """
    check(
        client.call("getblockcount") == height,
        f"the tip of the generated chain: {height}",
    )
    tip = client.call("getbestblockhash")
    check(tip == client.call("getblockhash", [height]), f"the tip hash: {tip}")
    check_serialization(client, tx_id, "a wallet transaction")
    coinbase = check_serialization(
        client, foreign_coinbase, "a transaction only -txindex can answer for"
    )
    # and that it is a coinbase: its one input names no previous output,
    # which Core reports as a `coinbase` member where every other input
    # carries a `txid`. So the transaction the index answered for is one
    # neither wallet could have been asked about
    check(
        "coinbase" in coinbase["vin"][0],
        "the transaction the index answered for is a coinbase",
    )


def check_rest(
    client: BitcoinCoreRpcClient, height: int, tx_id: str, foreign_coinbase: str
) -> None:  # pragma: no cover -- the -rest interface of a live node
...
) -> None:  # pragma: no cover -- the whole --chain run against a node
...
def main() -> int:
    ...
    with TemporaryDirectory(
    ...
    ) as tmp:  # pragma: no cover -- everything under it runs against a node
        datadir = Path(tmp)
        ...
        try:
            ...
            raise
    if args.protocol:  # pragma: no cover -- the report of a run that finished
        print(
    ...
    return 0  # pragma: no cover -- reached only after a node answered
