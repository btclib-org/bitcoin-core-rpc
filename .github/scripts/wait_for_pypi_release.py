...
first attempt instead -- which is what the `pull_request` trigger on
`pypi-install.yml` does, running this on the change that touches it and
getting the empty-tag answer. `tests/wait_for_pypi_release_test.py` is
...
is a step whose defect ships with a release.
    ...
    uv run --no-project --python 3.14 \
        .github/scripts/wait_for_pypi_release.py bitcoin-core-rpc "$TAG"
"""
