...
- **A trailing `#` comment inside a `shell` fence of `CLAUDE.md`,
  `CONTRIBUTING.md`, `RELEASING.md` or `REPOSITORY.md` no longer sits on
  the command line it followed** (issue btclib-org/.github#771): an
  interactive `zsh` leaves `INTERACTIVE_COMMENTS` unset, so it hands the
  comment's own words to the command as arguments rather than treating
  them as a comment. What the comment said either became a sentence
  above the fence or moved to a line of its own that opens with `#`,
  which section 9 at `btclib-org/.github`'s `5712e05` reads as harmless
  while it keeps to plain words, carrying neither an apostrophe nor a
  backtick. `CHANGELOG.md`'s two instances, inside a landed entry, stay:
  this file is append-only under `merge=union`, and a landed entry is
  the record of what was said when it landed.
- **`CLAUDE.md`'s `WT=` line loses its trailing worktree-name example**
  ...
  the parse before it was ever a command. The example moves into a
  sentence above the fence.
...
- **`RELEASING.md`'s `--subject <title>` loses its quoting and moves
  ahead of `--body-file <path>`, so a bare placeholder ends the
  command** (issue btclib-org/.github#775): section 9's unquoting bullet
  asks a placeholder standing as a whole argument to be bare, and its
...
- **`reasonless-coverage-pragma` refuses a `#`-comment `pragma: no cover`
  ...
  `btclib-org/.github`'s at `e1069d1`, and `types: [python]` reaches the
  smoke script, whose pragmas carry their reason on the line already, so
...
- **`check-changelog` refuses a `###` heading repeated in this file's
  ...
  `e1069d1` byte for byte, which section 14 owes every repository, and
  ...
  script's `_BLANK_LINE` carries the attribute docstring
  `check-docstring-first` reads as a second module docstring, so that
  hook's `exclude:` names the script beside the three `src/` modules it
  already names for the same shape, the script being owed byte for byte.
- **`toml-comment-width`'s `name:` states the pattern's own predicate,
  ...
  *`toml-comment-width`'s comment claims no other tool's amnesty*'s last
  bullet left the `name:` to a decision every tree carrying the hook
...
- **`.yamllint.yaml`'s `allow-non-breakable-words` comment states only
  ...
  and the file matches `btclib-org/.github`'s at `e1069d1` byte for
...
- **`codeql.yml`'s `codeql-passed` job asks the run's own job listing
  ...
  calls `test.yml` and `integration-bitcoind.yml`, so `test-passed` and
  `integration-passed` keep `needs` and only `codeql-passed` takes the
  listing. The job elevates to `actions: read` for the call, a read the
  write-only command `REPOSITORY.md`'s *Token permissions* hands the
  reader does not list, as it does not list `analyze`'s.
...
- **The step's comment cites btclib-org/btclib#1001 as what the listing
  ...
  goes with the loop. The entry above titled *Three gate aggregates take
  btclib's own allowlist shape* described `codeql-passed`'s loop over
...
- **`claude-review.yml`'s `mention` job says at its `pull-requests: write`
  ...
  line, and `REPOSITORY.md`'s *Token permissions* says of the four
  two-elevation jobs that the reason for every pair sits where it is
  declared; this grant carried none. Section 14 of `btclib-org/.github`'s
...
- **`.github/scripts/wait_for_pypi_release.py` is what
  ...
  drives it** (closes btclib-org/.github#509). A schedule and a dispatch
  ...
  that shape for this class. `btclib` and `btclib-secp256k1` carry the
  same script and the same test, and this is the last of the three trees
  the issue named.
- **The budget is a deadline and not a count of attempts times an
  ...
  multiplied out to well inside the timeout of the job that ran them. The
  deadline itself is the five minutes that loop named as its budget,
  chosen rather than measured -- nothing here times how long the index
  takes to serve an upload.
- **One job ahead of the matrix rather than a step inside the one that
  installs.** The question is about the index and not about a platform,
  so it is asked once; the install job still checks nothing out, which is
  what makes the import its own step verifies resolve to what pip
  installed. The wait job's checkout is sparse and reaches
  `.github/scripts`, and the script takes the standard library alone
  under `uv run --no-project`, so nothing of this project is built or
  installed to run it.
- **`[tool.ruff.lint.per-file-ignores]` exempts the script from
  `T201`**, as it does the two other scripts under `.github/scripts`
  that print: what it prints is what the log of the step running it
  shows, which is the case that rule is not about.
...
- **`.github/scripts/wait_for_readthedocs_build.py` is what
  ...
  `tests/wait_for_readthedocs_build_test.py` is what drives it** (closes
  ...
  job already has. `btclib` and `btclib-secp256k1` carry the same script
  and the same test, and this is the last of the three trees the issue
  named.
...
- **The file records `vars.CLAUDE_REVIEW_ENABLED`, the switch
  ...
  repository's `actions/variables` leaves the loop of facilities whose
