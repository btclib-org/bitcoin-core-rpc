...
_URL = "https://pypi.org/pypi/bitcoin-core-rpc/2026.9/json"
...
def test_the_version_asked_for_is_the_one_the_tag_names(
    ...
    assert clock.slept == []
    assert "the index serves 2026.9" in capsys.readouterr().out
...
def test_a_version_not_served_yet_is_waited_for(
    ...
    assert script.main(["bitcoin-core-rpc", "2026.9", "--interval", "15"]) == 0

    assert [url for url, _ in index.asked] == [_URL, _URL, _URL]
    assert clock.slept == [15.0, 15.0]
    assert "the index serves 2026.9 after 30 s" in capsys.readouterr().out
...
def test_a_version_that_never_arrives_is_the_deadline_speaking(
    ...
    assert script.main(["bitcoin-core-rpc", "2026.9", "--timeout", "300"]) == 1

    assert clock.now == pytest.approx(300.0)
    assert len(index.asked) == 20
    reported = capsys.readouterr().out
    assert "::error::2026.9 is not on the index 300 s later" in reported
...
def test_the_last_wait_is_cut_to_what_is_left_of_the_deadline(
    ...
    arguments = ["bitcoin-core-rpc", "2026.9", "--timeout", "20", "--interval", "15"]
...
def test_a_request_outlasting_the_deadline_leaves_no_negative_sleep(
    ...
    assert script.main(["bitcoin-core-rpc", "2026.9", "--timeout", "30"]) == 1

    assert asked == [(_URL, 10.0)]
    assert clock.slept == [0.0]
    assert "::error::2026.9 is not on the index 30 s later" in capsys.readouterr().out
...
def test_the_defaults_are_the_budget_a_release_actually_gets(
    ...
    assert script.main(["bitcoin-core-rpc", "2026.9"]) == 1
