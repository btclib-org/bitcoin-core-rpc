...
a schedule, on a dispatch and on the pull request that touches the
workflow the tag is empty, and the matrix installing anything at all is
the check that the script returned. What no run can show is any other
outcome -- what is waited on is somebody else's upload, so neither a
release nor a rehearsal can arrange for the index to be late -- so the
retry, the deadline and the `::error::` are reached here or nowhere.
...
_URL = "https://pypi.org/pypi/bitcoin-core-rpc/2026.9.1/json"
...
def test_an_empty_tag_is_nothing_to_wait_for(
    ...
    assert script.main(["bitcoin-core-rpc", ""]) == 0
...
def test_the_version_asked_for_is_the_one_the_tag_names(
    ...
    assert script.main(["bitcoin-core-rpc", "v2026.9.1"]) == 0
...
def test_a_version_not_served_yet_is_waited_for(
    ...
    assert script.main(["bitcoin-core-rpc", "2026.9.1", "--interval", "15"]) == 0
...
def test_a_version_that_never_arrives_is_the_deadline_speaking(
    ...
    assert script.main(["bitcoin-core-rpc", "2026.9.1", "--timeout", "300"]) == 1
...
def test_a_request_outlasting_the_deadline_leaves_no_negative_sleep(
    ...
    assert script.main(["bitcoin-core-rpc", "2026.9.1", "--timeout", "30"]) == 1
...
def test_the_defaults_are_the_budget_a_release_actually_gets(
    ...
    assert script.main(["bitcoin-core-rpc", "2026.9.1"]) == 1
