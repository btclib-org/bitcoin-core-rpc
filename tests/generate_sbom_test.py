...
def write_dist(
    ...
    *,
    ...
    members: tuple[str, ...] = ("bitcoin_core_rpc-2026.9.dist-info/METADATA",),
    version: str = "2026.9",
) -> tuple[Path, Path]:
    ...
    name, which is what `uv build` actually writes here and what
    `main`'s own `wheel.name.split("-")` has to survive: a fixture
    spelled with the dash instead would split into the wrong number of
    fields and read a truncated distribution name.
    """
    text = metadata + "".join(f"Requires-Dist: {r}\n" for r in requirements)
    wheel = directory / f"bitcoin_core_rpc-{version}-py3-none-any.whl"
    with zipfile.ZipFile(wheel, "w") as archive:
        for member in members:
            archive.writestr(member, text)
    sdist = directory / f"bitcoin_core_rpc-{version}.tar.gz"
...
def test_the_document_describes_the_distribution(
    ...
    assert component["name"] == "bitcoin-core-rpc"
    assert component["version"] == "2026.9"
    assert component["purl"] == "pkg:pypi/bitcoin-core-rpc@2026.9"
    assert component["licenses"] == [{"expression": "MIT"}]
    assert (
        component["description"] == "A standalone JSON-RPC client against Bitcoin Core"
    )
...
def test_the_two_files_are_named_with_their_digests(
    ...
    distributions = [
        r for r in references if r["url"].startswith("bitcoin_core_rpc-2026.9")
    ]
    assert [r["url"] for r in distributions] == [
        "bitcoin_core_rpc-2026.9-py3-none-any.whl",
        "bitcoin_core_rpc-2026.9.tar.gz",
    ]
...
def test_a_project_url_becomes_a_reference_of_its_kind(
    ...
    assert {
        "type": "website",
        "url": "https://bitcoin-core-rpc.readthedocs.io/",
        "comment": "homepage",
    } in references
    assert {
        "type": "vcs",
...
def test_submodule_components_is_a_no_op_for_this_repository(
    ...
    """This repository vendors no submodule, so a scan finds none."""
...
def test_the_dependency_graph_names_every_component(
    ...
    root, *leaves = document["dependencies"]
    assert root == {"ref": "pkg:pypi/bitcoin-core-rpc@2026.9", "dependsOn": refs}
...
def test_main_names_the_file_after_the_wheel(
    ...
    written = output / "bitcoin_core_rpc-2026.9.dev7.cdx.json"
...
def test_the_document_is_the_same_bytes_twice(
    ...
    name = "bitcoin_core_rpc-2026.9.cdx.json"
