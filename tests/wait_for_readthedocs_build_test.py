...
_URL = "https://bitcoin-core-rpc.readthedocs.io/en/v2026.9/"
_BUILDS = "https://app.readthedocs.org/projects/bitcoin-core-rpc/builds/"
_AGENT = "bitcoin-core-rpc-readthedocs-wait/1.0"
...
def test_the_page_asked_for_is_the_one_the_tag_names(
    ...
    assert script.main(["bitcoin-core-rpc", "v2026.9"]) == 0
...
def test_a_build_that_has_not_finished_is_waited_for(
    ...
    assert script.main(["bitcoin-core-rpc", "v2026.9", "--interval", "30"]) == 0
...
def test_a_build_that_never_arrives_is_the_deadline_speaking(
    ...
    assert script.main(["bitcoin-core-rpc", "v2026.9", "--timeout", "60"]) == 1

    assert clock.now == pytest.approx(60.0)
    assert len(site.asked) == 2
    reported = capsys.readouterr().out
    assert f"::error::tag v2026.9: {_URL} was never served in 60 s;" in reported
...
def test_a_deadline_already_spent_asks_nothing_and_still_annotates(
    ...
    assert script.main(["bitcoin-core-rpc", "v2026.9", "--timeout", "0"]) == 1
...
def test_a_request_outlasting_the_deadline_leaves_no_negative_sleep(
    ...
    assert script.main(["bitcoin-core-rpc", "v2026.9", "--timeout", "30"]) == 1
...
def test_the_defaults_are_the_budget_a_release_actually_gets(
    ...
    assert script.main(["bitcoin-core-rpc", "v2026.9"]) == 1
...
def test_the_site_answering_the_page_is_it_serving_the_tag(
    ...
    agent = script.USER_AGENT.format(project="bitcoin-core-rpc")
    assert agent == _AGENT
...
def test_the_main_guard_runs_the_script_as___main__(
    ...
    argument list or `script.unserved(...)` directly. `runpy.run_path`
    executes the file fresh with `__name__` set to `"__main__"`, which
    is what reaches that line, on a deadline of zero -- the one
    argument that asks the site nothing and returns at once.
    """
    monkeypatch.setattr(
        sys,
        "argv",
